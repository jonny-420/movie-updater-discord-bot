...
class BotRepo():
    ...
    async def insertGenreSubscription(self, genre, member):
        try:
            cursor = await self.connection.cursor()
            # This step already fetches the hashed member id from the database
            member_id = await self.getMemberId(cursor, member)
    ...
    async def listSubscriptions(self, member):
        try:
            cursor = await self.connection.cursor()
            member_id = await self.getMemberId(cursor, member)
    ...
    async def getMemberId(self, cursor, member):
        sql = f"SELECT id FROM member WHERE username = '{member.name}'"
        await cursor.execute(sql)
        return (await cursor.fetchall())[0][0]
    
    async def upcomingMoviesBySubscription(self, member):
        try:
            cursor = await self.connection.cursor()
            member_id = await self.getMemberId(cursor, member)

...
def etl():
    ...
    try:
        ...
        # Insert into DB (silently ignores duplicates)
        movie_stmt = insert(movie).values(movie_query).on_conflict_do_nothing(index_elements=['movie_id'])
        genre_stmt = insert(movieGenre).values(genre_query).on_conflict_do_nothing(index_elements=['movie_id', 'genre_id'])
